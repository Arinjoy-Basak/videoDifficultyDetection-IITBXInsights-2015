...
def printstatus(event):
	print str(event)
	print str(oldOVT)+"  "+str(oVT)+"  "+str(cVT)+"  "+str(timeWatched)+" "+flagPlayPause+" "+page
...
videosList=[('67a8559582864d6a8148e2ef5c997e8f',)]
#videosList=[('5957b78252cc43cfaddef910cf506dd5',)]

#Getting the video duration for an individual video in a single course
#query = ("select t.*, CV.videoUTubeId , CV.videolength from (select moduleSysName, count(eventName) eventCount from UserSessionOldLog where courseName=%s and moduleSysName is not null and eventType =%s group by moduleSysName order by eventCount desc limit 50) t, (select * from CourseVideos where courseName=%s) CV where t.moduleSysName=CV.videoSysName  order by eventCount desc limit 10")
#data = ('CS101.1x','video','CS101.1x')



"""
cursor.execute("select * from top50CSvid limit 10")
videosList=list()
for video in cursor:
...
#		timeFrameBuckets = [ [0,0.0] for x in range(0, int(math.ceil(duration/timeFrameSize)) ) ]
		...
		data  = ( str(user[0]), 'video', video[0], 'video', 'navigation', 'save_user_state', 'page_close' )
		cursor.execute(query, data)
		activity = list()
	...
	#XXX		if not activity[i-1][1]=='save_user_state':
				while not (i == len(activity) or (activity[i][1]=='save_user_state' and activity[i][3]<duration and activity[i][4]<duration ) ):	
			...
			#		print str(activity[ind][5])+" "+str(activity[ind-1][5])+" "+str(activity[i][5])+" "+str(activity[ind][5])
					if activityClean[-2][4] > activity[ind][4]:#	 and activity[ind][5]-activityClean[-2][5] < activity[i][5]-activity[ind][5]:
