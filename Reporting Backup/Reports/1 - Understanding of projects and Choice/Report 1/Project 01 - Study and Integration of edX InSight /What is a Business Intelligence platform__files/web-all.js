...
var BBEmailArticle = function(){
    ...
    return {
        ...
        EmailArticleClick : function(e, blogId, blogTitle){
            ...
            Recaptcha.create("6LdGBucSAAAAAJ21Mljrrt7UXk0ufIOazeTtO-6b",
                "emart-captcha-holder",
                {
                    theme: "white"
                 }
            );
        ...
        Submit : function(){
			...
			if (ErrorText != null) {
				...
				var PostData = '&bbAction=EmailArticleTo&BlogID=' + encodeURIComponent(BlogID) + '&bbEmailTo=' + encodeURIComponent(ToEmail.value) + '&bbEmailFrom=' + encodeURIComponent(FromEmail.value) + '&bbMessage=' + encodeURIComponent(Message.value);
                PostData += '&reCaptchaChallenge=' + encodeURIComponent(Recaptcha.get_challenge()) + '&reCaptchaResponse=' + encodeURIComponent(Recaptcha.get_response());
...
var fixDoubleFileUploadFields = function() {
  setTimeout(function(){
    jQuery('object[id^="upload_LeadGen_ContactForm_"]:eq(1)').remove();
  }, 1000);
};

AddEvent(window, 'load', fixDoubleFileUploadFields);